...
pub export fn zSetNextFocus() callconv(.c) void {
    if (df.inFocus != null)    {
        var wnd1 = df.inFocus;
...
pub export fn zSetPrevFocus() callconv(.c) void {
    if (df.inFocus != null) {
        var wnd1 = df.inFocus;
	var pwnd:df.WINDOW = null;
        while (true) {
...
pub export fn zReFocus(wnd:df.WINDOW) callconv(.c) void {
	if (df.GetParent(wnd) != null)	{
		zRemoveWindow(wnd);
		zAppendWindow(wnd);
		zReFocus(df.GetParent(wnd));
	}
}

// ---- remove a window from the linked list ----
pub export fn zRemoveWindow(wnd:df.WINDOW) callconv(.c) void {
    if (wnd != null)    {
...
pub export fn zAppendWindow(wnd:df.WINDOW) callconv(.c) void {
    if (wnd != null) {
        const pwnd = df.GetParent(wnd);
        if (pwnd != null) {
...
//              pass it on -------
pub export fn zSkipApplicationControls() callconv(.c) void {
    var EmptyAppl = false;
    var ct:isize = 0;
