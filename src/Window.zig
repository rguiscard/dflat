...
//    const wnd = df.cCreateWindow(klass, title, left, top, height, width, extension, parent, wndproc, attrib);
    ...
    if (wnd != null) {
        ...
        if (wndproc == null) {
            wnd.*.wndproc = Classes.classdefs[@intCast(klass)][1]; // wndproc
//            wnd.*.wndproc = classdefs[Class].wndproc;
        ...
        var base = klass;
        while (base != -1) {
            const cls = Classes.classdefs[@intCast(base)];
            const attr:c_int = @intCast(cls[2]); // attributes
            // df.AddAttribute(wnd, attr);
            wnd.*.attrib = wnd.*.attrib | attr;
            base = @intFromEnum(cls[0]);
        }
        var pt = parent;
        if (parent != null) {
...
//   This is to trasit from df.WINDOW to WINDOW.zig
pub export fn zSendMessage(wnd:df.WINDOW, msg:df.MESSAGE, p1:df.PARAM, p2:df.PARAM) c_int {
    const win:*TopLevelFields = @constCast(@fieldParentPtr("win", &wnd));
    const m:message = @enumFromInt(msg);
    return win.sendMessage(m, p1, p2);
}
